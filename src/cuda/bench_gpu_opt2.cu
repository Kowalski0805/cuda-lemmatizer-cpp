// bench_gpu_opt2.cu — improvement of bench_gpu_opt1 (remove prescan)
//
...
int main(int argc, char* argv[]) {
    ...
    char*       h_chars;
    CUDA_CHECK(cudaHostAlloc(&h_chars,   file_sz,                       cudaHostAllocDefault));
    ...
    rmm::device_uvector<char>          d_chars_raw(file_sz,    rmm::cuda_stream_default);
    ...
    offsets_vec.reserve(200000000);  // worst case: every 2 bytes = 1 char + newline
    ...
    while (p < end) {
        ...
        if (len > 0) {
            ...
            offsets_vec.push_back((int32_t)(pos += len));
            ++N;
        }
        while (p < end && (*p == '\n' || *p == '\r')) ++p;
    }
    ...
    int32_t*    h_offsets;
    ResultPair* h_out;
    CUDA_CHECK(cudaHostAlloc(&h_offsets, (N+1) * sizeof(int32_t),   cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc(&h_out,     N     * sizeof(ResultPair), cudaHostAllocDefault));
    memcpy(h_offsets, offsets_vec.data(), (N+1) * sizeof(int32_t));
    const size_t total_chars = (size_t)h_offsets[N];
    offsets_vec.clear();
    offsets_vec.shrink_to_fit();  // free the vector memory immediately
